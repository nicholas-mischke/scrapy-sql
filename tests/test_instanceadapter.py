...
# Package Imports
from scrapy_sql.instanceadapter import SQLAlchemyInstanceAdapter
...
class TestSQLAlchemyInstanceAdapter:
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                pytest.lazy_fixture(empty_Author),  # instance
                {
                    'item': pytest.lazy_fixture(empty_Author),
                    'item_class': Author,
    ...
    def test_dunder_init(self, instance, expected):
        request.getfixturevalue(dirname)


        assert SQLAlchemyInstanceAdapter(instance).__dict__ == expected
